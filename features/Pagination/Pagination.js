export class Pagination {
  ...
  constructor() {
    if (!Pagination.isntance) {
      Pagination.isntance = this;
      this.pagination = this.createPagination();
    }

    return Pagination.isntance;
  }
  ...
  createPagination() {
    ...
    const pagintaionSeparator = document.createTextNode(' из ');

    this.paginationTotal = document.createElement('span');
    this.paginationTotal.classList.add('pagination__total');

    paginationInfo.append(this.paginationCurrent, pagintaionSeparator, this.paginationTotal);
    paginationArrays.append(this.paginationLeft, paginationInfo, this.paginationRight);
  ...
  update({ currentPage, totalPages, totalProducts, limit }) {
    ...
    this.paginationCurrent.textContent = totalProducts === limit ? totalPages : width < totalProducts ? width : width - limit + (totalProducts % limit);
  ...
  mount(parrentElement, position) {
    if (position) {
      parrentElement.insertAjacentElement(position, this.pagination);
    }
    parrentElement.append(this.pagination);
