...
export class ProductCard extends BasicWrapper {

  static instance = null;

  constructor() {
    super('section', 'product');

    // Проверка наличия уже существующего образца
    if (!ProductCard.instance) {
      ProductCard.instance = this;
      this.containerElement = this.addContainer(['container', 'product__container']);
  ...
  getProductPicture(cardData) {
    ...
    const mainSliderItems = cardData.images.map(item => {
    ...
    if (cardData.images.length > 1) {
      const productArrowNext = document.createElement('div');
      ...
      const productArrowPrev = document.createElement('div');
    ...
    const productSliderThumbnailsList = document.createElement('div');
    productSliderThumbnailsList.classList.add('swiper-wrapper', 'product__thumbnails-list');

    const productThumbnailsItems = cardData.images.map(item => {
      ...
      productThumbnailsImage.classList.add('product__thumbnails-image');
    ...
    productSliderThumbnailsList.append(...productThumbnailsItems);
    productSliderThumbnails.append(productSliderThumbnailsList);
  ...
  getProductInfo(cardData) {
    ...
    productCharacteristicsTable.classList.add('product__characteristics-table', 'table');

    const productCharacteristicsRows = cardData.characteristics.map(item => {      
    ...
    const cardBtns = this.getProductBnts(cardData);

    productCharacteristics.append(productCharacteristicsTitle, ...productCharacteristicsRows);
    productInfo.append(productPrice, productArticle, productCharacteristics, cardBtns);

    return productInfo;
  };

  getProductBnts(cardData) {
    const productBtns = document.createElement('div');
    productBtns.classList.add('product__btns');

    const productBtn = new CartButton('product__btn', 'В корзину').create(cardData.id);

    const productLike = new LikeButton('product__like').create(cardData.id);
  ...
  render(cardData) {
    ...
    const cardTitle = this.getProductTitle(cardData);
    const cardSlider = this.getProductPicture(cardData);
    const cardInfo = this.getProductInfo(cardData);

    this.containerElement.append(cardTitle, cardSlider, cardInfo);
  }

  mount(parrentElement, cardData) {
