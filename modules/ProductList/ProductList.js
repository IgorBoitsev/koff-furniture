...
export class ProductList extends BasicWrapper {
  ...
  getHTMLTemplateListItem({ id, images: [image], name: title, price }) {
    return `
      <article class="goods__card card">
        <a href="/product/${id}" class="card__link card__link-image">
          <img src="${API_URL}${image}" alt="${title}" class="card__image">
        </a>
  ...
  getHTMLTemplatePagination(data) {
    if (data.length === 0) {
      return '<p>Ничего не найдено:(</p>';
    } else {
        return `
          <div class="pagination">
            <div class="pagination__bar" style="--width: calc(${data.length} / ${data.length} * 100%)">
            ...
            <div class="pagination__arrays">
              ...
              <p>
                <span class="pagination__current">${data.length}</span>
                из
                <span class="pagination__total">${data.length}</span>
              </p>
  ...
  updateListElem(data = [], listClassName, itemClassName) {
    ...
    this.containerElement.insertAdjacentHTML('beforeend', this.getHTMLTemplatePagination(data));
  };

  mount(parrentElement = '', sectionData, sectionTitle, sectionTitleClassName) {
